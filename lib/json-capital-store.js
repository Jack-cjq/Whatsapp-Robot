...
class JsonCapitalStore {
    ...
    configure({ filePath, backupDir, getMaxHistory, mergeWaitMs, latency } = {}) {
    ...
    async #maybeBackup(reason) {
        ...
        if (reason === 'startup' || this._lastBackupDay !== day) {
            ...
            try {
                ...
                setImmediate(() => this.#pruneBackups().catch(() => {}));
            } catch (e) {
                console.warn('⚠️ 备份 capital.json 失败:', e.message);
            }
        }
    }
