...
class FairGroupScheduler {
    ...
    /**
     * @param {string} chatId
     * @param {object} item { run: Function, enqueuedAtNs: bigint }
...
class AsyncLogger {
    ...
    async flush() {
        ...
        this.pendingWrite = this.pendingWrite.then(() => this.#writeBatch(batch)).catch(() => {});
...
class RuntimeMetrics {
    ...
    stop() {
        if (this.timer) { clearInterval(this.timer); this.timer = null; }
        if (this.histogram) {
            try { this.histogram.disable(); } catch (_) {}
            this.histogram = null;
        }
    }
...
class MemoryAccessControl {
    ...
    /**
     * 同时匹配数字 ID（含 @lid）与显示名（如 Tongyang / ~Tongyang）
     */
